    ...
    try {
        IFC2.init(() => {
            ...
            const polls = [
                ...
                "aircraft/0/systems/engines/0/state", "aircraft/0/systems/engines/1/state",
                "aircraft/0/systems/engines/2/state", "aircraft/0/systems/engines/3/state"
            ];
            ...
            polls.forEach(p => { 
                try { 
                    ...
                    if (!isCommandSupported(p)) {
                        console.log(`[PROXY] Skipping unsupported parameter for this aircraft: ${p}`);
                        return;
                    }
                    IFC2.pollRegister(p); 
                } catch(e) {
                    console.error(`[PROXY] Failed to register poll ${p}:`, e);
                } 
            });
        }, { host: ip, port: 10112 });
    } catch(e) {
...
setInterval(() => {
    if (isConnected && Date.now() - lastDataTime > 8000) {
        console.log("[PROXY] Connection dropped silently (no incoming data for 8s). Resetting...");
        isConnected = false;
