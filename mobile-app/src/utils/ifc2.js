 ...
 */
const dgram = require('react-native-udp'); // For listening for UDP broadcasts
const net = require('react-native-tcp-socket'); // For establishing socket connections
...
const IFC2 = {
    ...
    getCommand: (cmd, args) => {
        ...
        const u8Command = Buffer.from(abCommand);
    ...
    setCommand: (cmd, val) => {
        ...
        switch (cmdType) {
            ...
            case IFC2.STRING:
                ...
                dvCommand.setInt32(5, strBuf.length, true);
                for (let i = 0; i < strBuf.length; i++) {
                    dvCommand.setUint8(9 + i, strBuf[i]);
                }
                break;
            case IFC2.LONG:
    ...
    processManifest: () => {
        ...
        for (const key in manifestLines) {
            ...
            const name = lineData[2] ? lineData[2].trim() : lineData[2]; // Get the command name
